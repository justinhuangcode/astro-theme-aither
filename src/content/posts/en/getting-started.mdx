...
| `ui.defaultMode` | Default theme: `'light'`, `'dark'`, or `'system'` |
...
The theme supports three modes: Light, Dark, and System (follows OS preference). The default is `system`. Change it in `src/config/site.ts`:
...
Theme switching uses the View Transitions API for a smooth circular reveal animation.
...
The included GitHub Actions workflow handles CI/CD automatically.
...
**Step 2** — Push to GitHub. The workflow will:

1. Build the site on every push and PR
2. Deploy to Cloudflare Pages on push to `main`
...
This project uses [CalVer](https://calver.org/) — `YYYY.0M.0D`. Same-day patches append a suffix: `2026.03.14.1`, `2026.03.14.2`, etc.
